...
class StripeCheckoutHandler {
  ...
  /**
   * Create Customer Portal session with proper return URL configuration
   */
  async createCustomerPortalSession(options = {}) {
    try {
      ...
      const {
        returnUrl = callbackUrl || `${window.location.origin}/dashboard.html`,
        flowData = {}
      } = options;

      // Generate idempotency key for safe retries
      const idempotencyKey = `portal-${user.id}-${Date.now()}`;

      // Create portal session via API endpoint
      const response = await fetch('/api/create-portal-session.js', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${await this.getAuthToken()}`,
      ...
      this.notifyListeners('portal_session_created', {
  ...
  async redirectToCustomerPortal(options = {}) {
    try {
      ...
      this.notifyListeners('portal_redirect', {
...
window.createCustomerPortalSession = async function(options = {}) {
  if (!window.stripeCheckoutHandler) {
    window.stripeCheckoutHandler = new StripeCheckoutHandler();
  }
  return await window.stripeCheckoutHandler.createCustomerPortalSession(options);
};

window.redirectToCustomerPortal = async function(options = {}) {
  try {
    const portalResult = await window.createCustomerPortalSession(options);
