-- Quick setup script for your Supabase project
...
CREATE INDEX IF NOT EXISTS idx_user_files_tool_type ON user_files(tool_type);
CREATE INDEX IF NOT EXISTS idx_usage_analytics_user_id ON usage_analytics(user_id);
CREATE INDEX IF NOT EXISTS idx_usage_analytics_created_at ON usage_analytics(created_at);
CREATE INDEX IF NOT EXISTS idx_user_preferences_user_tool ON user_preferences(user_id, tool_type);
...
DROP POLICY IF EXISTS "Users can view own subscriptions" ON payment_subscriptions;
CREATE POLICY "Users can view own subscriptions" ON payment_subscriptions
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Service role can manage subscriptions" ON payment_subscriptions;
...
DROP POLICY IF EXISTS "Service role can manage analytics" ON usage_analytics;
...
DROP POLICY IF EXISTS "Service role can manage usage" ON monthly_usage;
...
DROP TRIGGER IF EXISTS handle_updated_at_payment_subscriptions ON payment_subscriptions;
CREATE TRIGGER handle_updated_at_payment_subscriptions
  BEFORE UPDATE ON payment_subscriptions
  FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

DROP TRIGGER IF EXISTS handle_updated_at_user_files ON user_files;
CREATE TRIGGER handle_updated_at_user_files
  BEFORE UPDATE ON user_files
  FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

DROP TRIGGER IF EXISTS handle_updated_at_user_preferences ON user_preferences;
CREATE TRIGGER handle_updated_at_user_preferences
  BEFORE UPDATE ON user_preferences
  FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

DROP TRIGGER IF EXISTS handle_updated_at_monthly_usage ON monthly_usage;
